...
inner_cut_super = .55 * inner_layer_diam_super
...
def generateSTL(input_array,dummyCylinder=False,outerDiam=-1,extrusion=-1,fname='cylinder',selectionType="random",metadata_in=metadata):
    (N_Layers, M_PillPerLayer, outer_layer_diam_mm, inner_layer_diam_mm, outer_pillar_cutoff_mm,
     ...
     l_over_deltaT_A, pillar_mode) = metadata_in.values()
    metadata["pillar_mode"] = selectionType
    with open(fname+".json", "w") as file:
        json.dump(metadata_in, file, indent=4)
    l = top_face_z
    # Replace BCs
    if dummyCylinder:
    ...
    if selectionType == "innerouter":
        ...
        for xi in range(-ind_range,ind_range+1):
            for yi in range(-ind_range,ind_range+1):
                for bi in range(2):
                    ...
                    d2 = x ** 2 + y ** 2
                    # print(close)
                    # print(str((d2,pillar) not in close))
                    if is_valid(np.array([x,y]),inner_pillar_cutoff_mm,outer_pillar_cutoff_mm) and (d2,pillar) not in close:
                        close.append((d2,pillar))
        close_list = sorted(close, key=lambda x: x[0])
        for layer in range(N_Layers-1):
            for ii in range(M_PillPerLayer):
                ...
                # print(ii)
                if layer % 2 == 0:
                    x,y = get_xy(close_list[ii][1])
                else:
                    x,y = get_xy(close_list[-1-ii][1])
                print(x,y)
            ...
            for _ in range(M_PillPerLayer):
                while True:
                    ...
                    if val:
                        ...
                        pill = cq.Workplane("XY").circle(pillar_radius_mm).extrude(pillar_height_mm).translate((x,y,layer_thickness_mm+layer*(pillar_height_mm+layer_thickness_mm)))
                        structure = structure.union(pill)
                        break
    elif selectionType == "zigzag":
        l1 = []
        l2 = []
        for i in np.arange(-ind_range,ind_range+2,2):
            pass
...
def computeFEMFlux(vtk_file,foldernamein):
    import os
    logfile = "FEM_output"
    ...
    with open(file_path, 'r') as file:
        content = file.read()
    content = re.sub(r'filename_mesh\s*=\s*".*\.vtk"', f'filename_mesh = "{foldernamein}\{vtk_file}"', content)
...
if doGenerateNostack:
    ...
    for i in range(Nstages):
        ...
        loTA = generateSTL(None,fname=pathname,selectionType="random",metadata_in=metadata)
        if doRemesh:
            sname = pathname + ".step"
            meshS = .3
            generateMesh(sname, meshSize=meshS, meshMin=meshS / 2, meshMax=2 * meshS)
