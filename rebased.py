...
def generateSTL(input_array,dummyCylinder=False,outerDiam=-1,extrusion=-1,fname='cylinder',selectionType="random",metadata_in=metadata,foldername=None,intermediateName=None):
    ...
    hex_vectors = np.array([[np.sqrt(3)/6,.5],[1./np.sqrt(3),1]])
    ...
    def get_xy(tup):
        n,m,p = tup
        return cella*(n*basis_vectors[0]+m*basis_vectors[1]+hex_vectors[0]+p*(hex_vectors[1]-hex_vectors[0]))
    ...
    if selectionType == "innerouter":
            ...
            for _ in range(M_PillPerLayer):
                while True:
                    ...
                    if val:
                        ...
                        break
            pillar_list.append(layer_list)
    elif selectionType == "r_zigzag1":
        l1 = []
        l1c = []
        l2 = []
        l2c = []
        for xi in range(-ind_range, ind_range + 1,3):
            for yi in range(-ind_range, ind_range + 1,3):
                for bi in range(2):
                    bi = 0 # TODO
                    pillar = [xi, yi, bi]
                    x, y = get_xy(pillar)
                    ...
                    if is_valid(np.array([x, y]), inner_pillar_cutoff_mm, outer_pillar_cutoff_mm*1.1) and pxy not in l1c:
                        ...
                        lpxy.append(tuple(pillar))
                        l1.append(lpxy)
        for xi in range(-ind_range-1, ind_range + 1, 3):
            for yi in range(-ind_range-1, ind_range + 1, 3):
                for bi in range(2):
                    bi = 0 # TODO
                    pillar = [xi, yi, bi]
                    x, y = get_xy(pillar)
        ...
        for layer in tqdm(range(N_Layers - 1)):
            ...
            for ii in range(lengthOfLists):
                ...
                structure = structure.union(pill)
            pillar_list.append(layer_list)
    elif selectionType == "r_zigzag2":
        l1 = []
        l1c = []
        l2 = []
        l2c = []
        for xi in range(-ind_range, ind_range + 1,4):
            for yi in range(-ind_range, ind_range + 1,2):
                for bi in range(2):
                    ...
                    xii, yii, d2i = np.round(x, 2), np.round(y, 2), np.round(d2, 2)
                    pxy = (xii, yii, d2i)
                    if is_valid(np.array([x, y]), inner_pillar_cutoff_mm, outer_pillar_cutoff_mm*1.05) and pxy not in l1c:
                        l1c.append(pxy)
                        lpxy = list(pxy)
                        lpxy.append(tuple(pillar))
                        l1.append(lpxy)
        for xi in range(-ind_range-2, ind_range + 1, 4):
            for yi in range(-ind_range, ind_range + 1, 2):
                for bi in range(2):
                    bi = 0 # TODO
                    pillar = [xi, yi, bi]
                    ...
                    xii, yii, d2i = np.round(x, 2), np.round(y, 2), np.round(d2, 2)
                    pxy = (xii, yii, d2i)
                    if is_valid(np.array([x, y]), inner_pillar_cutoff_mm, outer_pillar_cutoff_mm*1.05) and pxy not in l2c:
                        l2c.append(pxy)
                        lpxy = list(pxy)
                        lpxy.append(tuple(pillar))
                        l2.append(lpxy)
...
if doGenerateNostack:
    ...
    for i in range(Nstages):
        ...
        loTA = generateSTL(None,fname=pathname,selectionType="r_zigzag4",metadata_in=metadata,foldername="",intermediateName="intermediatestage_2024_02_19_10_23")
        # if doRemesh:
        #     sname = pathname + ".step"
        #     meshS = .15
        #     generateMesh(sname, meshSize=meshS, meshMin=meshS / 2, meshMax=2 * meshS)
